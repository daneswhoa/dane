...
const contractorDir = path.join(__dirname, 'apps/portal/app/contractor');

// Create directories
['jobs', 'network', 'earnings'].forEach(dir => {
  const p = path.join(contractorDir, dir);
...
export function ContractorProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ContractorContext.Provider value={{ profile, jobs, loading, errorMsg, setErrorMsg, fetchData }}>
      {children}
...
`;
fs.writeFileSync(path.join(contractorDir, 'ContractorContext.tsx'), contextContent);
...
function ContractorLayoutInner({ children }: { children: React.ReactNode }) {
  const { profile, jobs, loading, errorMsg, setErrorMsg, fetchData } = useContractor();
  const { data: session, isPending } = useSession();
...
`;
fs.writeFileSync(path.join(contractorDir, 'layout.tsx'), layoutContent);

// page.tsx redirects to /contractor/jobs
const rootPageContent = `import { redirect } from 'next/navigation';
export default function ContractorRoot() {
  redirect('/contractor/jobs');
}
`;
fs.writeFileSync(path.join(contractorDir, 'page.tsx'), rootPageContent);

console.log('Restructured basic layout');
