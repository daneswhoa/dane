...
import { 
  ...
  Loader2, HelpCircle, LogIn, UserPlus, Sun, Moon
} from 'lucide-react';
...
function InvitePageContent() {
  ...
  const handleSignOutAndRedirect = async () => {
    try {
      await signOut();
      window.location.href = `/login?redirect=${encodeURIComponent(getRedirectUrl())}`;
    } catch (e) {
    ...
    try {
      ...
      setTimeout(() => {
        if (isTenant) {
          ...
          window.location.href = `${dashboardUrl}/overview`;
        }
      }, 2000);
    } catch (err: any) {
  ...
  const renderTopbar = () => (
    <header className="fixed top-0 left-0 right-0 h-16 bg-white/70 dark:bg-ink-955/70 backdrop-blur-md border-b border-paper-100 dark:border-ink-800 z-50 flex items-center justify-between px-6 sm:px-12 transition-colors duration-300">
            ...
            {session && inviteDetails && session.user?.email?.toLowerCase() !== inviteDetails.email?.toLowerCase() ? (
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <a
                    href={`/login?redirect=${encodeURIComponent(getRedirectUrl())}`}
                  ...
                  <a
                    href={`/register?redirect=${encodeURIComponent(getRedirectUrl())}`}
